...
int AtoDmV(uint8_t *r, int lsbBit, float *outADVolt);
...
int AtoDmV(uint8_t *recive, int lsbBit, float *outADVolt)
{
	int adVolt;
	
	//下位8bitのみと次のデータを使用(取得データの下位bitは精度の問題で捨てる)
	adVolt = ( (recive[1]&0xF)<<8 | recive[2] ) >>lsbBit;
	
	if( outADVolt != NULL )
		*outADVolt	= adVolt * ((float)VREF / ( 1<<(AD_RESOLUTION-lsbBit) ));
	return adVolt;
}
//6.5-7.1us前後
int PinGetADCH(int pin, int ch, float *outADVolt, unsigned long sleepTime, unsigned int lsbBit)
{
	...
	adVolt = AtoDmV(rbuf, lsbBit, outADVolt);
